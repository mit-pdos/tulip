...
func main() {
	...
	for gid, addrm := range(gaddrm) {
		for rid, addr := range(addrm) {
			...
			txnpaxos.Start(rid, addr, fname, addrmpx, fnamepx)
	...
	for {
		fmt.Printf("[main] Enter command: write <key> <value> | delete <key> | read <key> | dumpall\n> ")
		...
		if err == nil {
			for gid, addrm := range(gaddrm) {
				for rid, addr := range(addrm) {
					...
					conn := ret.Connection
					data := txnpaxos.EncodeDumpStateRequest(gid)
